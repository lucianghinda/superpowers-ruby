...
### Rails Upgrade Skill Caught Up To Rails 8.1.3.1 and 8.2

- **Five wrong facts corrected** — the skill claimed Rails 8.1 *removed* the built-in Sidekiq adapter. It only deprecated it; removal is 8.2. **SuckerPunch** is the adapter 8.1 actually removes. The two sit adjacent in the same 8.1 release-note block, one "Deprecate" and one "Remove", and collapsing them sent readers chasing a break that does not exist while missing the one that does. Also fixed: `pool:` in `database.yml` was a **rename** to `max_connections` with the old key still working, not a removal (demoted HIGH → LOW); the form helpers `text_area`/`check_box` were never deprecated, only aliased; `postgresql_adapter_decode_dates` is a **7.2** default, not 8.1; and `load_defaults 8.0` is `Regexp.timeout = 1` plus `strict_freshness`, not the three settings previously listed.
- **Rails 8.0 → 8.1 re-rated from Easy to Medium** — 8 documented changes became ~28, and 2-4 hours became 1-2 days. The under-count had a specific cause: the official upgrading guide lists exactly *one* item for this hop, so anything sourced from the guide alone misses the release notes and per-component CHANGELOGs where the real removals live.
- **The biggest 8.x change was missing entirely** — `ActiveRecord::MissingRequiredOrderError`. Under `load_defaults 8.1`, `#first`/`#last` raise on an unordered relation whose model has no fallback ordering. It arrives as a wave of test failures rather than a single boot error, which is why the skill now says to enable that flag alone and treat the failure list as the work queue.
- **CVE-2026-66066 is now a workflow gate, not a grep** — the critical Active Storage file-read/RCE fixed in 8.1.3.1 disables libvips "unfuzzed" loaders, and libvips below 8.13 cannot block them at all, so **Rails refuses to boot**. That version lives in a Dockerfile or base image, so no amount of searching the application finds it. It became Step 0.5 of the workflow, alongside the fact that BMP/ICO/PSD variants now raise and that an app exposed while vulnerable needs its `secret_key_base` and every reachable credential rotated.
- **Rails 8.2 documented, clearly marked unreleased** — 8.2 is `8.2.0.alpha` on `main`. It earns coverage because 8.1's deprecation warnings name 8.2 as the removal version, so an 8.1 warning cannot be triaged without it, and because every 8.2 fix already works on 8.1. Covers the Ruby **3.3.1** floor, the Active Record 6.1 marshalling-format removal (flush caches *before* upgrading), negative enum scopes now including `nil` records, the SQLite 3.35.0 / PostgreSQL 10.0 minimums, and the `Sec-Fetch-Site` CSRF strategy — ending with an ordering that clears the whole 8.2 HIGH list from 8.1.
- **One 8.1 feature was already reverted** — alphabetical `schema.rb` column sorting shipped in 8.1 and is reverted on `main`, because it produces improper production tables under `db:prepare`. Recorded in both directions, since a skill tracking only releases would teach a behavior Rails has abandoned.
- **`fetch-changelogs.sh` could not fetch security releases** — its validator allowed at most three version segments, so `./fetch-changelogs.sh 8.1.3.1` was rejected outright. The version an upgrade should target today was the one version the script refused. Now accepts 2-4 segments.
- **`fetch-changelogs.sh main` added** — fetches the unreleased entries that describe the next Rails version. Main-branch changelogs have no version headers to anchor on, since Rails empties them when a release branch is cut, so the new extractor reads from the top, stops defensively at any `## Rails` header, strips the trailing "check x-y-stable for previous changes" pointer, and returns empty for components with nothing pending. Output is stamped with a warning that entries can still change or be reverted.

Every correction was verified against `rails/rails` on 2026-08-13, using `guides/source/configuring.md` for `load_defaults` values — those lists are generated from the framework's own defaults, so unlike prose they cannot drift from the code.
...
- **writing-plans** — replaced Plan Review Loop with inline Self-Review checklist: spec coverage, placeholder scan, type consistency
...
- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full superpowers bootstrap at session start. (Original fix by @culinablaz in upstream PR #910)
...
- **codex-tools** — added named agent dispatch mapping documenting how to translate Claude Code's named agent types to Codex's `spawn_agent` with worker roles (upstream PR #647 by @arittr)
